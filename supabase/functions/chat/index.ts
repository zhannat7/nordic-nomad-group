...
function wasAskingAge(messages: any[]): boolean {
  const last = [...messages].reverse().find((m) => m.role === "assistant");
  if (!last) return false;
  const c = last.content.toLowerCase();
  return (
    c.includes("сколько вам лет") ||
    c.includes("жашыңыз канчада") ||
    c.includes("канча жаш") ||
    c.includes("жашыңыз") ||
    c.includes("сколько лет")
  );
}
...
САМОЕ ВАЖНОЕ:
- "Тахмина" — это ТВОЁ имя. Имя пользователя НЕИЗВЕСТНО. Никогда не обращайся к нему по имени — только "вы".
- Ты уже поздоровалась. НИКОГДА больше не пиши "Здравствуйте".
...
ЭҢ МААНИЛҮҮСҮ:
- "Тахмина" — бул СЕНИН атың. Колдонуучунун аты БЕЛГИСИЗ. Аны ЭЧКАЧАН ат менен атабайсың — жөн гана "сиз".
- Сен бир жолу саламдаштың. ЭЧКАЧАН кайра "Саламатсызбы" жазбайсың.
...
serve(async (req) => {
  ...
  try {
    ...
    if (wasAskingAge(messages.slice(0, -1))) {
